...
You can also read raw data from TOS and write it directly to a Lance table using the same configuration:
...
=== "🐍 Python"
    ...
    # Read source data stored in TOS (for example, a Parquet dataset)
    df = daft.read_parquet("s3://my-tos-bucket/raw/events/", io_config=io_config)

    # Write the data as a Lance table back to TOS
    meta = df.write_lance("s3://my-tos-bucket/lance/events.lance", io_config=io_config)
    meta.show()
    ```
